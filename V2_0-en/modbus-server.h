...
#if HAVE_REVSH
# define HTONS(_val) __REVSH(_val)
# define NTOHS(_val) __REVSH(_val)
#else
# define HTONS(_val) (uint16_t)(((_val) & 0xFF) << 8 | (_val) >> 8 & 0xFF)
# define NTOHS(_val) (uint16_t)(((_val) & 0xFF) << 8 | (_val) >> 8 & 0xFF)
#endif  /* !HAVE_REVSH */
...
typedef enum {
  ...
  MB_CONFIGURE = 0x65                       /**< User-defined device configuration */
} mb_function_code_t;
...
/* ============================================================================
 *    Modbus structure type definitions and forward declarations
 * ============================================================================ */
 ...
 * @details Is used to bind external data sources to a Modbus instance.
 *          Supports multiple Modbus instances sharing the same data source
 *
 * @warning Concurrency protection precautions:
 *          There is a risk of race conditions when multiple Modbus instances
 *          share the same data source. Recommended measures:
 *          1. For atomic operations of 16 bits and below, the STM32 Cortex-M
 *             core guarantees atomicity.
 *          2. For complex operations involving multiple registers, critical
 *             section protection should be used.
 *          3. Simple critical sections can be implemented using __disable_irq()/__enable_irq().
 *          4. Alternatively, use the mutex mechanism of the RTOS.
 */
typedef struct MB_DataMap {
  /* Coils - Modbus address 0xxxx */
  uint8_t *coils;                   /**< Coil data pointer (bit-wise storage). */
  uint16_t coil_count;              /**< Number of coils (0 - 65535) */

  /* Discrete Inputs - Modbus address 1xxxx */
  uint8_t *discrete_inputs;         /**< Pointer to discrete input data (bit-wise storage). */
  uint16_t discrete_count;          /**< Number of discrete inputs (0 - 65535). */

  /* Holding Registers - Modbus address 4xxxx */
  uint16_t *holding_registers;      /**< Pointer to holding registers (16-bit storage). */
  uint16_t holding_register_count;  /**< Number of holding registers (0 - 125). */

  /* Input Registers - Modbus address 3xxxx */
  uint16_t *input_registers;        /**< Pointer to input registers (16-bit storage). */
...
typedef struct MB_RS485Config {
  bool enabled;                     /**< RS485 mode enable flag */
  GPIO_TypeDef *de_port;            /**< DE/RE control pin GPIO port (e.g., GPIOA) */
  uint16_t de_pin;                  /**< DE/RE control pin number (e.g., GPIO_PIN_8) */
  bool de_polarity;                 /**< DE polarity: true: reset de_pin for RX; false: set
                                         de_pin for RX. */
} MB_RS485Config_t;

/**
 * @brief   Modbus server initialization configuration structure
 * @details All configuration items are passed through this structure at
 *          runtime. Replaces the static macro definition method of V1.0
 */
typedef struct MB_Config {
  /* UART configuration */
  UART_HandleTypeDef *huart;        /**< HAL UART handle */
  uint8_t server_addr;              /**< server address (valid range: 1~247) */
  uint32_t baud_rate;               /**< baud rate (used for dynamic timeout calculation) */

  /* Data mapping */
  MB_DataMap_t data_map;            /**< Data area mapping configuration */

  /* Hardware interface configuration */
  MB_RS485Config_t rs485;           /**< RS485 configuration (enabled=false when not in use) */

  /* Runtime options */
  bool use_dma_tx;                  /**< Send using DMA (requires CubeMX DMA config) */
  bool use_hw_crc;                  /**< Use hardware CRC calculation (requires CubeMX CRC config) */
  bool use_crc_table;               /**< Uses CRC lookup table */
} MB_Config_t;
...
typedef struct MB_ServerHandle {
  ...
  /* =================== CRC Interface =================== */
  UART_HandleTypeDef *hcrc;       /**< HAL UART handle */

  /* ==================== Server Configuration =================== */
  uint8_t server_addr;              /**< Current server address */
  uint32_t baud_rate;               /**< Current baud rate (used for timeout calculation) */
  uint8_t parity;                   /**< Current parity setting (0: none, 1: odd, 2: even) */

  /* ==================== Ping Pong Buffer Zone ==================== */
  uint8_t req[2][MB_RTU_ADU_MAX];   /**< Ping-pong Modbus request buffers */
  ...
  /* ==================== Data Mapping =================== */
  MB_DataMap_t data_map;            /**< Data area pointer mapping */

  /* ==================== Hardware Configuration =================== */
  MB_RS485Config_t rs485;           /**< RS485 configuration */

  /* ==================== Runtime Options =================== */
  bool use_dma_tx;                  /**< Send using DMA (requires CubeMX DMA configuration) */
  bool use_hw_crc;                  /**< Use hardware CRC calculation (requires CubeMX CRC
                                         configuration) */
  bool use_crc_table;               /**< Uses CRC lookup table (fast, requires 512B ROM) */

  /* ==================== Callback Function =================== */
  MB_ConfigCallback_t config_cb;    /**< 0x64 callback*/
  MB_WriteCallback_t write_cb;      /**< Write callback */
